...
# Note: You can also use model IDs directly: "deepseek-v4-flash/latest", etc.
MODELS = [
...
def call_model(model: str, user_prompt: str) -> str:
    """Call Yandex Cloud chat-completions endpoint."""
    if not API_KEY:
        raise RuntimeError(
    ...
    # Yandex Cloud uses a slightly different API structure
    # The messages format is the same as OpenAI
    payload = {
        "model": yandex_model,
    ...
    # The chat completions endpoint is at /foundationModels/v1/completion
    # Actually, Yandex uses /v1/completion or /v1/chat/completions
    # Let's use the standard OpenAI-compatible endpoint
    ...
    try:
        with urllib.request.urlopen(request, timeout=180) as response:
            ...
            if VERBOSE_LOGGING:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    content_preview = result["choices"][0]["message"]["content"][:200]
                    print(f"[API RESPONSE] Success - Preview: {content_preview}...")
                else:
                    print(f"[API RESPONSE] Success - Raw: {json.dumps(result, indent=2)[:200]}...")
        ...
        if VERBOSE_LOGGING:
            ...
            print(f"[API ERROR] Response body: {error_body}")
        raise
